...
def numsafe(thestring):
    """ a stupid function to replace numbers in a string with letters """
    return str(thestring).replace('0','zero').replace('1','one').replace('2','two').replace('3','three').replace('4','four').replace('5','five').replace('6','six').replace('7','seven').replace('8','eight').replace('9','nine').replace('-','').replace('/','').replace(',','')
...
def writetex(thefile,thename,thestring):
    ...
    * thestring: the string to write as the macro value
	...
	nl=thename+'}'
	os.system("grep -n \\"+thename+"} "+thefile+r" | grep -v '% \\newcommand' | sed -ne 's/ *:.*//p' | xargs -I nx sed -i 'nx s/^/% /' "+thefile)
	# Write new line
	f=open(thefile,'a')
	timestamp='  % '+datetime.datetime.today().strftime('%Y %B %d %H:%M:%S.%f')
	f.write('\\newcommand{\\'+thename+'}{'+str(thestring)+'}'+timestamp+'\n')
	f.close()
    else:
...
def textransfer(file_in,file_out,thename):
    ...
    * thename: The name of the macro
    ...
    os.system("grep -n \\"+thename+"} "+file_out+r" | grep -v '% \\newcommand' | sed -ne 's/ *:.*//p' | xargs -I nx sed -i 'nx s/^/% /' "+file_out)
    ## now transfer the line
    os.system("cat "+file_in+r" | grep -v '^%' | grep '\\"+thename+"}' >> "+file_out)
...
def LCticks(axs,ynbins=7,myval=0.5):
    ...
    * myval: Magnitude spacing for y-axis
    ...
    the_xrange=[list(axs.ravel()[i].axis())[0:2] for i in range(len(axs.ravel()))]
    for i in range(len(axs.ravel())): 
	axs.ravel()[i].ticklabel_format(style='plain',axis='both',useOffset=False)
	axs.ravel()[i].xaxis.set_major_locator(matplotlib.ticker.MaxNLocator(nbins=5,prune='both'))
	axs.ravel()[i].xaxis.set_minor_locator(matplotlib.ticker.FixedLocator(arange(the_xrange[i][0]-100,the_xrange[i][1]+100,5)))
	axs.ravel()[i].yaxis.set_major_locator(matplotlib.ticker.MaxNLocator(nbins=ynbins,prune='both'))
	axs.ravel()[i].yaxis.set_minor_locator(matplotlib.ticker.FixedLocator(arange(-30,30,myval)))
...
def PrettyTicks(axs,N=[5,5],mf=3,xprune='both',yprune='both'):
	...
	axs[i].xaxis.set_major_locator(matplotlib.ticker.MaxNLocator(nbins=N[0],prune=xprune))
	axs[i].xaxis.set_minor_locator(matplotlib.ticker.MaxNLocator(nbins=N[0]*mf,prune=xprune))
	axs[i].yaxis.set_major_locator(matplotlib.ticker.MaxNLocator(nbins=N[1],prune=yprune))
	axs[i].yaxis.set_minor_locator(matplotlib.ticker.MaxNLocator(nbins=N[1]*mf,prune=yprune))
...
def bandf(mag,magerr,band,ftype=None,mode='lambda',units='cgs',z=0.,N=1e4):
    """
    ...
    if ftype==None:
        ...
        elif band in ['U','V','B','R','I','J','H','K','Y']: ftype='Vega'
        elif 'GHz' in band: ftype='muJy'
        else: ftype='AB'
    mag_MC=random.normal(mag,magerr,N)
    if ftype=='AB':
        if mode=='lambda': 
            flux=10**((mag_MC+48.574)/-2.5)*(2.998e18)/(allbands[band][0])**2
        elif mode=='nu': 
        ...
        if mode=='lambda': 
            ...
            zp=allbands[band][3]
            flux=10**((mag_MC)/-2.5)*zp*(2.998e18)/(allbands[band][0])**2
...
def CCMextinct(l,EBmV,Rv=3.1,mag=0):
    ...
    * mag: Return A_\lambda (mag) instead of flux multiplier
    ...
    a=zeros(len(x));b=zeros(len(x))
    ### IR -> Optical
    sel=where((x>0.3) & (x<1.1))
    a[sel]=0.574*x[sel]**1.61
    b[sel]=-0.527*x[sel]**1.61
    sel=where((x>1.1) & (x<3.3))
    a[sel]=1+0.17699*y[sel]-0.50447*y[sel]**2-0.02427*y[sel]**3+0.72085*y[sel]**4+0.01979*y[sel]**5-0.77530*y[sel]**6+0.32999*y[sel]**7
    b[sel]=1.41338*y[sel]+2.28305*y[sel]**2+1.07233*y[sel]**3-5.38434*y[sel]**4-0.62251*y[sel]**5+5.30260*y[sel]**6-2.09002*y[sel]**7
    ### UV and FUV
    Fa=-0.04473*(x-5.9)**2-0.009779*(x-5.9)**3
    Fb=0.2130*(x-5.9)**2+0.1207*(x-5.9)**3
    sel=where(x<5.9)
    Fa[sel]=zeros(len(sel[0]));Fb[sel]=zeros(len(sel[0]))
    sel=where((x>3.3) & (x<8))
    a[sel]=1.752-0.316*x[sel]-0.104/((x[sel]-4.67)**2+0.341)+Fa[sel]
    b[sel]=-3.090+1.825*x[sel]+1.206/((x[sel]-4.62)**2+0.263)+Fb[sel]
    sel=where((x>8) & (x<10))
    a[sel]=-1.073-0.628*(x[sel]-8)+0.137*(x[sel]-8)**2-0.070*(x[sel]-8)**3
    b[sel]=13.670+4.257*(x[sel]-8)-0.420*(x[sel]-8)**2+0.374*(x[sel]-8)**3
...
def rsig(x):
    """
    A robust "standard deviation" estimator - returns 1/2 of the distance between the 84th and 16th percentiles of a sample
    """
    m=percentile(x,[16,84])
    return (m[1]-m[0])/2.
...
def pgrepv(tfile,outfile=None,hcharacter='#',checkcol=6):
	...
	lines2 = []
	for line in lines:
	    if line[0] != hcharacter and line!='\n' and (checkcol==0 or (line[0:5] =='lp__,' or len(line.split(',')[checkcol-1].strip())==1)):
		lines2.append(line)
    outdata = ''.join(lines2)
    if outfile==None:
	return outdata
...
def myloadcsv(fn):
	...
	for line in fno.readlines():
		...
		if len(data) != len(header):
		    print('myloadcsv '+fn+': line #'+str(i)+' is bad, ignoring')
		else:
		    ##Record data
...
def SNplotMLC(name,bands=['g','r','i','z','y','FUV','NUV_APER'],UV_base=1,ax=None,usedetlims=1,addtype=1,addDM=0):
    """
    Plot multi-band light curve data for PS1 SNe with data in aphot
    
    Parameters:
    * name: SN name (i.e. '2011-E-160042')
    * bands: List of photometric bands to plot (e.g. ['g','r'] - all grizy and FUV/NUV by default)
    * UV_base: Plot baseline levels for GALEX photometry
    * ax: Use pre-establish axes, or None to create new
    ...
    * addtype: Add the SN type from aphot to the plot title
    * addDM: Add distance modulus (plot abs mag; default = 0)
    """
    ##Establish plot
    if ax==None:
	fig,ax=texfig()
    ...
    ##Estimate bounds
    if addDM: dm=distmod(z)
    else: dm=0
    sel=where((aphot['event']==name) & (aphot['band']=='r') & (aphot['limit']==0))
    dmax=aphot['MJD'][sel][argmin(aphot['m'][sel])]
    vmax=aphot['m'][sel][argmin(aphot['m'][sel])]+dm
    
    ## Draw labels
    ptitle=pso_shortname_dict[name]+(': '+aphot['type'][sel][0])*addtype
    ax.text(.1,1.1,ptitle,fontsize=18,transform = ax.transAxes)
    ax.set_xlabel('MJD')
    ax.set_ylabel('m (AB mag)')
    
    ##Plot photometry
    for band in bands:
	if band in ['FUV','NUV_APER']: cs=2
	else: cs=1
	## Detections
        sel=where((aphot['event']==name) & (aphot['band']==band) & (aphot['limit']==0))
        if len(sel[0])>0: ax.errorbar(aphot['MJD'][sel],aphot['m'][sel]+dm,aphot['dm'][sel],color=bandc(band[0:3],col=1),mec='none',capsize=0,fmt='.')
        ax.plot(aphot['MJD'][sel],aphot['m'][sel]+dm,'o',label='$'+band.replace('_APER','')+'$',color=bandc(band[0:3],col=1),mec='none',ms=cs*5)
        ## Upper limits
        sel=where((aphot['event']==name) & (aphot['band']==band) & (aphot['limit']==1))
        if usedetlims:
	    ax.plot(aphot['MJD'][sel],array([PS1_detlims[band]]*len(sel[0]))+dm,'v',color=bandc(band[0:3],col=1),mec='none',ms=cs*3)
	else:
	    ax.plot(aphot['MJD'][sel],aphot['m'][sel]+dm,'v',color=bandc(band[0:3],col=1),mec='none',ms=cs*3)
    
    ## Set plot limits
    ax.set_ylim([ax.get_ylim()[1],vmax-2])
    ax.set_xlim([ax.get_xlim()[0]-10,ax.get_xlim()[1]+50])
    ax.xaxis.set_major_locator(MultipleLocator(50))
    
    ##Plot NUV/FUV baseline
    if UV_base:
	for band in ['NUV','FUV']:
	    sel=where((aphot['event']==name) & (aphot['band']==band) & (aphot['limit']==0) & (abs(dmax-aphot['MJD'])>50))
	    if len(sel[0])>0:
		bm=aphot['m'][sel]
		ax.fill_between(ax.get_xlim(), array([percentile(bm,16)]*2)+dm,array([percentile(bm,84)]*2)+dm,color='b',alpha=.7)
	
    ##Legend
    ax.legend(numpoints=1,bbox_to_anchor=(.3,1,.7,.1),ncol=5,prop={'size':9})
...
def texUpDown(x,n=2):
	...
	if n < 0:
	    ...
	    n=0
	fstring = '0.'+str(n)+'f'
	return ('%'+fstring)%x[1]+'^{+'+('%'+fstring)%x[2]+'}_{-'+('%'+fstring)%x[0]+'}'
...
Nugent_d=np.genfromtxt('/home/nes/Research/SNe/PSOs/IIps/Nugent/sn2p_flux.v1.2.dat',names=['t','l','f'])
...
BESSfc={}
for b in 'UBVRI':
    BESSfc[b]=np.genfromtxt(rootdir+'literature/Bessel/bess-'+b.lower()+'.pass',names=['l','f'])
...
def calc_Nug_color(band1,band2,Nday=61.,tilt=0.,altspec=None):
	...
	filt1 = interp1d(PS1fc[band1]['l'],PS1fc[band1]['f'],fill_value=0,bounds_error=0)
	mod1=0
    else:
	filt1 = interp1d(BESSfc[band1]['l'],BESSfc[band1]['f'],fill_value=0,bounds_error=0)
	mod1 = VegaF[band1]
    if band2.lower()==band2:
	filt2 = interp1d(PS1fc[band2]['l'],PS1fc[band2]['f'],fill_value=0,bounds_error=0)
	mod2=0
    else:
	filt2 = interp1d(BESSfc[band2]['l'],BESSfc[band2]['f'],fill_value=0,bounds_error=0)
	mod2 = VegaF[band1]
    ## Tilt it
	...
	Nx = Nugent_d['l'][Nsel]; Ny = Nugent_d['f'][Nsel]
    else:
	Sdata = np.genfromtxt(altspec,names=['l','f'])
	Nx = Sdata['l']; Ny = Sdata['f']
    SNIIPf = interp1d(Nx, Ny * (Nx)**2 * 10**tilt_f(tilt,Nx),fill_value=0,bounds_error=0)
    color = -2.5 * log10( sum(filt1(ls)*SNIIPf(ls)/sum(filt1(ls))) / sum(filt2(ls)*SNIIPf(ls)/sum(filt2(ls)))) + mod1 - mod2
    return color

def calc_Nug_Scor(band1,band2,c_band1,c_band2,target_color,Nday=61.,altspec=None):
    """
    ...
    """
    opt_tilt_f = lambda x: target_color - calc_Nug_color(c_band1,c_band2,Nday=Nday,tilt=x,altspec=altspec)
    opt_tilt = fsolve(opt_tilt_f, 0)
    Scor = calc_Nug_color(band1,band2,Nday=61.,tilt=opt_tilt,altspec=altspec)
    return Scor


###### K corrections
VI_color_law = lambda test_color,x: ((test_color / 0.1)*-0.121)*(x-10000)/10000
def Kcor_IIP(b,z,t,test_color=0, A_V=None):
    ...
    * A_V: Extinction to apply to the Nugent template
    ...
    N_l=np.zeros(N_N);N_f=np.zeros(N_N)
    N_l=Nugent_d['l'][where(Nugent_d['t']==0.)]
    t1=Nugent_t[bisect.bisect(Nugent_t,t)-1] # the last Nugent_t value less than t
    t2=Nugent_t[bisect.bisect(Nugent_t,t)]
    f1=Nugent_d['f'][Nugent_d['t']==t1] # the flux corresponding to N_l at t1
    ...
    return -2.5*log10(1/(1+z) * sum(l*f(l)*R)/sum(l*f(l*(1+z))*R))
...
bsort=lambda x: allbands[x][0]
bandsort=sort(allbands,)

def bandc(band,col=0,offset=1500.,scale=9500.):
    """
    Returns central wavelength and width ([c,w] in Angstroms) for the specified band
    
    Uses \lambda_eff from:
    optical: Table 1 of Bessell+05 (2005ARA&A..43..293B)
    UV: http://www.mssl.ucl.ac.uk/www_astro/uvot/uvot_instrument/filterwheel/filterwheel.html
    
    Parameters:
    * band: Filter name
    * col: if col=1, returns a python RGB color tuple instead of the central wavelength
    * offset: offset for color scale (the wavelength of bkue in Angstroms)
    * scale: width of color scale (scale+offset = wavelength of red in Angstroms)
    """
    ##Load values
    [cw,width]=allbands[band][0:2]
    ##Return color or not?
    if col: output=cm.jet((cw-float(offset))/float(scale))
    else: output=[cw,width]
    return output


# Extinction correction factors for grizy, relative to A_V
# Taken from NED calculations for PS1 filters (using SN 2010ay host as template)
A_factors = array([0.054,0.038,0.028,0.022,0.018])/0.046 

##Planck function - cgs
c_h=6.6260755e-27
c_c=2.99792458e10
c_kB=1.380658e-16
c_KeV=1.1604519e4  ## Kelvin per eV
c_Rsun=6.96e10 # cm
c_pc=3.086e18 #cm
fplanck=lambda T,l: 2*c_h*c_c**2/l**5/(exp(c_h*c_c/(l*c_kB*T))-1)

def NS_planck(L,T,l):
    """
    Blackbody flux following Ehud Nakar's Lnu.m
    """
    ## Convert l in Angstroms to nu in Hz
    nu=c_c/(l*1e-8)
    ## Constants
    numax=3*c_kB*T/c_h
    Lnumax=L/numax/1.53
    ## Calculate Lnu
    if T<1e4: Lnu=0  ##Why do this?
    else: Lnu=Lnumax*(nu/numax)**3*(exp(c_h*numax/(c_kB*T))-1)/(exp(c_h*nu/(c_kB*T))-1)
    ## Convert to Llambda
    return Lnu/l**2*3e18

def NS_smooth():
    """
    Function to smooth over light travel time differences
    """
    theta=linspace(0,pi/2.,100)
    tcross=R*500*c_Rsun/c_c
    ts=t-tcross*(1-cos(theta))
    ## # NOT DONE
    
    return -trapz(x=cos(theta),y=tmp)

def NS_RSGm(t,l,M,R,E):
    """
    Nakar & Sari RSG luminosity model
    
    Parameters:
    * t: Array of times in days
    * l  Array of wavelengths in Angstroms
    * M: Mass in units of 15 solar masses
    * R: Radius in units of 500 solar radii
    * E: Energy in units of 10^51 erg
    """
    ## Setup
    t_0=(300/60./60./24.)*M**.21*R**2.16*E**-0.79 #days
    t_s=14/24.*M**.43*R**1.26*E**-0.56 # days
    L_ti=4e43*M**-0.37*R**2.46*E**.3*(t*24)**-(4/3.)
    T_i=25*c_KeV*M**-0.3*R**-0.65*E**0.5
    ## Initialize output vectors
    L=np.empty(len(t))
    T=np.empty(len(t))
    ## Calculate values in each regime
    
    
    ## return flux
    return NS_planck(L,T,l)


def mSBO_NS(p,t,l,s,z):
    """
    Nakar + Sari 2010 RSG shock breakout model with parameters p
    
    p=[t0,M,R,E,Av] (see punloadSBO_NS)
    * t0: Shock breakout time, MJD
    * M: Mass in solar units
    * R: Radius in solar units
    * E: Energy in units of ergs (NOT log)
    * Av: V-band extinction (mag)
    
    Returns array of fluxes at wavelengths l and times t for filters of width s.
    """
    ## Extract parameters from p
    [t0,M,R,E,Av]=punloadSBO_NS(p)
    n=1.5
    ## apply redshift
    t=(t-t0)/(1+z)
    l=l/(1+z)
    s=s/(1+z)
    ## Calculate the transition time (Eq. 30)
    ts=(14/24.)*M**.43*R**1.26*E**-0.56 # days
    ## Calculate temperature (Eq. 31)
    T_1=10*c_KeV*M**-.22*R**.12*E**.23*(t*24.)**-.36 # Kelvin, planar phase
    T_2=3*c_KeV*M**-.13*R**.38*E**.11*t**-.56 # Kelvin, spherical phase
    ## Calculate radius for each phase
    R_1=R*500 # see S2.2.3
    R_2=R*500*(t/ts)**((5.31*n**2+9.09*n+4.25)/((1.19*n+1)*(5.58*n+4.25))) # Eq. 20
    #R_2=R*500*(t/ts)**((0.81*n+1)/(1.19*n+1)) # Eq. 5
    #pdb.set_trace()
    #Decide when it's in which phase
    phase_s=(t>ts)*1
    fT=T_1*(1-phase_s)+T_2*phase_s
    fR=R_1*(1-phase_s)+R_2*phase_s
    ##Planck function
    #fF=lambda t,l: 4*pi*((fR*c_Rsun)**2*fplanck(fT,l*1e-8))
    fF=lambda l,s,T,R: mean(4*pi*((R*c_Rsun)**2*fplanck(T,linspace(l-s/2.,l+s/2.,1000)*1e-8)))
    vfF=vectorize(fF)
    ##Calculate fluxes for each specified time
    fluxes=vfF(l,s,fT,fR)*1e-8 #convert from erg/s/cm to erg/s/AA
    ##Extinction
    efluxes=fluxes/CCMextinct(l,Av/3.1,Rv=3.1)
    ##Output 0 flux before t0
    efluxes[where(t<0)]=0
    #pdb.set_trace()
    return efluxes
...
def punloadSBO_NS(p):
    """
    Unload and rescale parameters for Nakar & Sari SBO model
    """
    t0=p[0]
    M=p[1]/15.
    R=p[2]/500.
...
def pSBO_NS(p,t,l,s,f,df,z):
    """
    Posterior probability function to evaluate Nakar & Sari SBO model
    as compared to lightcurve data
    
    See also emceeSBO_NS, MCMC sampler function
    """
    ## Likelihood
    #Call the model
    mf=mSBO_NS(p,t,l,s,z)
    diff=f-mf
    #calculate likelihood
    like=-sum((diff/df)**2/2.)-0.5*log(2*pi*df**2)
    ## Priors
    priors=[]
    [t0,M,R,E,Av]=punloadSBO_NS(p)
    tt=linspace(min(t),max(t),20)
    # Truncated Gaussian prior on M=15+-10 Msol
    sM=M*15
    if sM<1: priors.append(-inf)
    else: priors.append(-log(10.)-(sM-15.)**2/(2.*10**2)) 
    # Truncated Gaussian prior on R=500+-400 Msol
    sR=R*500
    if sR<10: priors.append(-inf)
    else: priors.append(-log(400.)-(sR-500.)**2/(2.*400**2)) 
    # Gaussian prior on log E = 51+-2
    sE=log10(E)+51
    if sE<49: priors.append(-inf)
    else: priors.append(-log(2.)-(sE-51.)**2/(2.*2**2)) 
    # Prior to keep t0 < min(t)
    if t0>min(t): priors.append(-inf)
    # Strong prior on Av, truncated exponential with scale lambda=2
    if Av<0.001: priors.append(-inf)
    else: priors.append(log(2)-2*Av)
    #pdb.set_trace()
    return like+sum(priors)


def emceeSBO_NS(t,l,s,f,df,z,nwalkers=250,burn=100,samples=500,threads=5):
    """
    Use emcee to fit the Sari & Nakar SBO model given lightcurve observations
    
    Uses pSBO_NS function as likelihood
    
    Parameters:
    * t: List of lightcurve point times (any units)
    * l: List of lightcurve point central wavelengths (Anstroms)
    * s: List of lightcurve point widths (Angstroms)
    * f: List of lightcurve point fluxes (ergs/s/cm^2/AA)
    * z: Redshift of object (for calculating absolute flux, time dilation/redshifting)
    
    Optional parameters:
    * df: List of lightcurve point flux errors
    * nwalvers: Number of emcee walkers
    * burn: Length of burn in
    * samples: Number of emcee samples for each walker
    * lowextinct: Use a flat prior so A_V<lowextinct (in mag)
    """
    import emcee
    # Make times relative, de-redshift wavelengths and convert to cm, calculate absolute fluxes
    t=array(t);l=array(l);s=array(s);f=array(f);df=array(df)
    l=l/(1+z)/1e8;s=s/(1+z)/1e8
    area=4*pi*cosmocalc(z)['DL_cm']**2
    f=f*area;df=df*area
    # Choose an initial set of positions for the walkers.
    offset=array([min(t),15,500,1e51,0.1])
    scale=array([5,3,100,.3e51,0.01])
    ndim=len(offset)
    p0 = [random.normal(offset,scale,ndim) for i in xrange(nwalkers)]
    # Initialize the sampler with the chosen specs.
    sampler = emcee.EnsembleSampler(nwalkers, ndim, pSBO_NS, args=[t,l,s,f,df,z], threads=threads)  
    # Run 500 steps as a burn-in.
    pos, prob, state = sampler.run_mcmc(p0, burn)
    # Reset the chain to remove the burn-in samples.
    sampler.reset()
    # Starting from the final position in the burn-in chain and continue sampling
    sampler.run_mcmc(pos, samples, rstate0=state)
    return sampler



#############################################################
######## Rstan light curve model
#############################################################

def skewnormal(x,m=0.,s=1.,a=0.):
    t = (x-m) / s
    return 2. / s * scipy.stats.norm.pdf(t) * scipy.stats.norm.cdf(a*t)

def emceeSPII(t,m,dm,lim,z,nwalkers=250,burn=100,samples=500,threads=5,t_0=None):
    """
    Use emcee to fit a simple exponential + linear light curve model for SN IIP
    
    
    Details:
    * With respect to time, fitting is done in the rest frame.
    * To be applied to a single band of photometry
    * Upper limits are incorporated in the Bayesian fit
    
    Parameters:
    * t: List of lightcurve point times (MJD)
    * m: List of lightcurve point magnitudes (apparent mag)
    * m: List of lightcurve point magnitude uncertainties (mag)
    * lim: Boolean array of length len(m) specifying whether each point is an upper limit or not
    * z: Redshift of object (for calculating absolute flux, time dilation/redshifting)
    ...
    * burn: Length of burn in
    * samples: Number of emcee samples for each walker
    * threads: Number of parallel threads to launch for emcee
    * t_0 - Guess at explosion date MJD (if None, it will pick the first detection)
    """
    import emcee
    #Ensure array type
    m=array(m);t=array(t);lim=array(lim)
    # Estimate time of peak
    if t_0==None: guess_t_0=min(t[where(lim==0)])
    else: guess_t_0=t_0
    # Apply distance modulus
    m=m+distmod(z)
    # Choose an initial set of positions for the walkers.
    offset=array([guess_t_0,-18,0.8,0.01,0.2,mean(m),std(m),0.2**2])
    scale=array([10,1,.1,.05,0.02,.1,.1,0.05**2])
    ndim=len(offset)
    p0 = [random.normal(offset,scale,ndim) for i in xrange(nwalkers)]
    # Initialize the sampler with the chosen specs.
    sampler = emcee.EnsembleSampler(nwalkers, ndim, pSPII, args=[t,m,dm,lim,z], threads=threads)  
...
def pSPII(p,t,m,dm,lim,z):
    """
    Posterior probability function for the simple SN IIP light curve model
    
    Uses a Gaussian mixture model for foreground ("good") and background ("bad" / outlier) points
    
    m = M_p + e^(-alpha * t') + (beta * t')
    
    * m = absolute magnitudes
    * M_p = peak magnitude (~-18 mag)
    * t' = (t - t_0)/(1+z)
    * alpha = Rise factor (~0.8)
    * beta = Decline slope (~0.01)
    * dm = distance modulus (mag)
    
    Additionally, there are some parameters controlling outlier rejection.  See section 3 of arXiv:1008.4686 by Hogg et al.
    
    V is the intrinsic scatter (see Eq. 35 of Hogg et al.)
    
    See also emceeSPII, MCMC sampler function
    """
    ### Extract the parameters
    t_0,M_p,alpha,beta,Pb,Yb,Vb,V=p[0:8]
    
    ## Likelihood
    #Define the model
    mf=mSPII(p,t,z)
    #calculate likelihood for detections    
    sel=where(lim==0)
    diff=(m-mf)**2/(dm**2+V)
    plike_1 = log(1-Pb) + (-(diff[sel])/2. - .5*log(2*pi*(dm[sel]**2+V))) ## good points - detections only
    plike_2 = log(Pb) + (-(m-Yb)**2/(2*(Vb+dm**2)) - .5*log(2*pi*(Vb+dm**2))) ## bad points - all
    like=log(sum(exp(plike_1))+sum(exp(plike_2)))
    
    ## Evaluate probability to be an outlier
    q=qSPII(p,t,m,dm,z)
    
    ## Priors
    priors=[]
    # Force 0 < alpha < 10 (i.e. rise at early times)
    if alpha<=0 or alpha>=10: priors.append(-inf)
    # Force 0 < beta < 1 (i.e. reasonable decline rate)
    if beta<=0 or beta>=1: priors.append(-inf)
    # Force t_0 to be before a detection and within 100 days of a detectiom (i.e. reasonable explosion date)
    if t_0>=max(t[where(lim==0)]) or min(abs(t_0-t[where(lim==0)]))>100: priors.append(-inf)
    # Force model to lie below "good" upper limits
    sel = where((lim==1) & (q==1))
    if sum(mf[sel] < m[sel])>0: priors.append(-inf) # careful with the direction of that inequality!    
    # Weakly informative prior on the probability to be an outlier, Pb
    poutly_2=log(scipy.stats.beta.pdf(Pb,1,7))
    priors.append(poutly_2)
    # Normal prior on the mean of the distribution of the magnitude outliers,
    nd_mean=mean(m[where(lim==0)])
    nd_std=std(m[where(lim==0)])
    poutly_3=-((nd_mean-Yb)/nd_std)**2/2.-.5*log(2*pi*nd_std**2)
    priors.append(poutly_3)
    # Flat prior on the log of the width in the outlier magnitude distribution
    poutly_4=-((log10(nd_std**2)-log10(Vb))/2)**2/2.-.5*log(2*pi*1**2)
    priors.append(poutly_4)
    # Weakly informative prior on log of the intrinsic scatter
    pintr=-((log10(0.2**2)-log10(V))/2)**2/2.-.5*log(2*pi*.1**2)
    priors.append(pintr)
    # Could also add reasonable prior on M_p 
    #pdb.set_trace()
    return like+sum(priors)
...
def mSPII(p,t,z):
    ...
    * Av: V-band extinction (mag)
    ...
    Returns array of magnitudes with respect to rest frame time
    """
    ## Extract parameters
    t_0,M_p,alpha,beta=p[0:4]
    ### Rescale the times
    ct=(t-t_0)/(1+z)
    return M_p+exp(-alpha*ct)+beta*ct

def qSPII(p,t,m,dm,z):
    """
    Calculate probability for a light curve point to be part of the background distribution 
    """
    t_0,M_p,alpha,beta,Pb,Yb,Vb,V=p
    mf=mSPII(p,t,z)
    p_fg=scipy.stats.norm.pdf(m,mf,sqrt(dm**2+V))
    p_bg=scipy.stats.norm.pdf(m,Yb,sqrt(dm**2+Vb))
    q=p_bg/(p_fg+p_bg)
    return q
...
def pystan_SPII_tm(SNdata,stanmodel=(rootdir+'stan/SIIP_t_multilevel.stan'),chains=10,iterations=1000,Kcor=None,get_neff=0,stan_init='0',verbose=0,parallel=1,fixhyper=0,dump_data=0,fluxoffset=1,**kwargs):
    """
    ...
    * fluxoffset: Divide fluxes by 1e7 before passing to Stan?
    ...
    global compiled_stan_model
    with open(stanmodel,'r') as obj: modeltext=obj.read()
    if compiled_stan_model != None and compiled_stan_model[0]==hashlib.md5(modeltext).hexdigest():
	print "pystan_SPII_tm: Past compiled model found and model file unchanged..."
	past_fit=compiled_stan_model[1]
    elif compiled_stan_model != None and compiled_stan_model[0]!=hashlib.md5(stanmodel).hexdigest(): 
	past_fit=None
	...
	for i in range(len(SNdata)): dp_all[i]=dp[where(events==SNdata['event'][i])[0]]
	#sel=where(sel_b & (SNdata['MJD']>(dp_all-dpeak_a)) & (SNdata['MJD']<(dp_all+dpeak_b)))
	SNdata2=[]
	for i in range(len(events)): 
	    SNdata2.append(pystan_SPII_tm_getLC(events[i],dp[i],bands='grizy'))
	SNdata2=np.hstack(SNdata2)
	## Exclude null fluxes
	sel=where(SNdata2['raw_flux']!=0)
	SNdata2=SNdata2[sel]
	## Get redshift list
	z=[SNdata2['z'][where(SNdata2['event']==event)[0][0]] for event in events]
	## Get zero point lists
	old_mzs=np.zeros([len(events),5])
	for k in range(len(events)):
	    for j in range(5):
		try: old_mzs[k,j]=get_r_mzero(events[k],dp[k],band='grizy'[j])
		except: old_mzs[k,j]=0
	mzs=fix_mzs(old_mzs)
	## Get filter list
	...
	Kdic={}
	for i in range(1,1+len(events)): Kdic[events[i-1]]=i
	for i in range(len(SNdata2)): K[i]=Kdic[SNdata2['event'][i]]
	## Check Kcor list
	if Kcor==None:
    ...
    if stan_init=='myprior': stan_init=pystan_SPII_tm_init # backwards compatibility
    if stan_init==0 or stan_init=='0':
	sinit=0
    elif stan_init=='random':
	sinit='random'
    elif hasattr(stan_init, '__call__'): ## check that a function has been passed
	if fixhyper:
	    null,hyperinit=stan_init(len(events),fixhyper=1)
	    for key in hyperinit: y_in[key]=hyperinit[key]
	    sinit=lambda: stan_init(len(events),fixhyper=2)
	else:
	    sinit=lambda: stan_init(len(events),fixhyper=0)
    else: raise Exception("Unrecognized init mode")
    
    #### Do sampling
    fit_params={"data":y_in, "iter":iterations, "chains":chains, "verbose":verbose, "init":sinit, "n_jobs":parallel}
    for key in fit_params: kwargs[key]=fit_params[key]
    ...
    if get_neff==1:
	all_neff = [pystan.chains.ess(fit.sim, i) for i in range(len(fit.sim['fnames_oi']))]
	...
	for i in range(len(fit.sim['fnames_oi'])):
	    try: all_rhat[i] = pystan.chains.splitrhat(fit.sim, i)
	    except ZeroDivisionError: all_rhat[i]=nan
	for k, v in zip(fit.sim['fnames_oi'], all_rhat): neffs['rhat_'+k]=v
	ndiv = [fit.get_sampler_params()[c]['n_divergent__'] for c in range(chains)]
...
def pystan_SPII_tm_ind(SNdata,stanmodel=(rootdir+'stan/SIIP_t_multilevel_mod5_lin_ind.stan'),chains=10,iterations=1000,Kcor=None,get_neff=0,stan_init='0',verbose=0,parallel=1,dump_data=0,fluxoffset=1,**kwargs):
    """
    ...
    * fluxoffset: Divide fluxes by 1e7 before passing to Stan?
    ...
    global compiled_stan_model_ind
    with open(stanmodel,'r') as obj: modeltext=obj.read()
    if compiled_stan_model_ind != None and compiled_stan_model_ind[0]==hashlib.md5(modeltext).hexdigest():
	print "pystan_SPII_tm: Past compiled model found and model file unchanged..."
	past_fit=compiled_stan_model_ind[1]
    elif compiled_stan_model_ind != None and compiled_stan_model_ind[0]!=hashlib.md5(stanmodel).hexdigest(): 
	past_fit=None
	print "pystan_SPII_tm: Past model found, but model file has changed; must recompile"
    else: 
	print "pystan_SPII_tm: No past model found model; must recompile"
    ...
    old_mzs=np.zeros([5])
    for j in range(5):
	try: old_mzs[j]=get_r_mzero(name,dp,band='grizy'[j])
	except: old_mzs[j]=0
    mzs=fix_mzs(old_mzs)
    ## Get filter list
    J=intbands(SNdata2['band'])
...
def pystan_SPII_tm_getLC(name,dpeak,bands='grizy',btwdays=50.,remove_dupes=0):
  """
  ...
  * remove_dupes: If >0, remove any duplicate points, using this value as the window (in days) to identify them
  ...
  for b in bands:
	...
	band_out_sel=where((sIDs==SNsID) | (sIDs==SNsID-1) | (sIDs==SNsID+1))
	## add to output
...
def pystan_SPII_tm_getfit(sout,name):
    ...
    s_fit=fit.copy()
    if 'alpha' in s_fit: 
	fix_keys=['pt0','t1','tp','t2','td','alpha','beta1','beta2','betadN','betadC','Yb','V','Mp','M1','M2','Md','mpeak','t0'] #,'Mpr'
	for key in fix_keys:
	    if len(shape(s_fit[key])) == 3:
		s_fit[key] = s_fit[key][:,:,sid] # note that new linear model has N_SN and N_F switched, as of 3/2014
	    else:
		s_fit[key] = s_fit[key][:,sid]
    elif 'lalpha' in s_fit: 
	fix_keys=['pt0','t1','tp','t2','td','lalpha','lbeta1','lbeta2','lbetadN','lbetadC','Yb','V','Mp','M1','M2','Md','mpeak','t0'] #,'Mpr'
	for key in fix_keys:
	    s_fit[key] = s_fit[key][:,sid]
    return s_fit
...
def pystan_SPII_tm_mod_init(Nsn,Nfilt=5,fixhyper=0):
    """
    Generate reasonable initial parameters for the SNIIP modified hierarchical light curve model
    ...
    * fixhyper: Unused
    ...
    params['Yb']=random.normal(0,100,[Nsn,Nfilt])
    params['V']=abs(random.normal(100,10,[Nsn,Nfilt]))
    params['P_hf']=random.normal(0,.1,[10,Nfilt])
    params['sigma_hf']=abs(random.normal(.5,.05,[10]))
    params['P_hSN']=random.normal(0,.1,[10,Nsn])
    params['sigma_hSN']=abs(random.normal(.5,.05,[10]))
    params['P_hSNf']=random.normal(0,.1,[10,Nfilt,Nsn])
    params['sigma_hSNf']=abs(random.normal(.5,.05,[10]))
    params['P_hh']=[
	random.normal(log10(3),.1), # 1 - t1
	random.normal(log10(80),.1), # 1 - t2
	random.normal(2,.1), # 1 - tp
	random.normal(1,.1), # 1 - td
	random.normal(-2,.1), # 1 - lalpha
	random.normal(-2,.1), # 1 - lbeta1
	random.normal(-1.7,.1), # 1 - lbeta2
	random.normal(-2,.1), # 1 - lbetadN
	random.normal(-2,.1), # 1 - lbetadC
	random.normal(0,.1), # 1 - Mpr
	]
    params['sigma_hh']=abs(random.normal(.5,.05,[10]))
    return params



def pystan_SPII_tm_mod2_init(Nsn,Nfilt=5,fixhyper=0):
    """
    Generate reasonable initial parameters for the SNIIP modified hierarchical light curve model
    ...
    * fixhyper: Unused
    """
    params={}
    params['pt0']=random.normal(-40,20,Nsn)
    params['Yb']=random.normal(0,100,[Nsn,Nfilt])
    params['V']=abs(random.normal(100,10,[Nsn,Nfilt]))
    params['P_hf']=random.normal(0,.1,[10,Nfilt])
    params['sigma_hf']=abs(random.normal(.5,.05,[10]))
    ...
    params['sigma_hh']=abs(random.normal(.5,.05,[10]))
    params['P_hPf']=random.normal(0,.1,[10,Nfilt,10])
    params['sigma_hPf']=abs(random.normal(0.1,.01,[10]))
    return params



def pystan_SPII_tm_mod3_init(Nsn,Nfilt=5,fixhyper=0):
    """
    Generate reasonable initial parameters for the SNIIP modified hierarchical light curve model
    that also has explosion date pooling
    ...
    * fixhyper: Unused
    ...
    params={}
    params['Yb']=random.normal(0,100,[Nsn,Nfilt])
    params['V']=abs(random.normal(100,12,[Nsn,Nfilt]))
    params['P_hf']=random.normal(0,.1,[12,Nfilt])
    params['sigma_hf']=abs(random.normal(.5,.05,[12]))
    params['P_hSN']=random.normal(0,.1,[13,Nsn])
    params['sigma_hSN']=abs(random.normal(.5,.05,[13]))
    params['P_hSNf']=random.normal(0,.1,[12,Nfilt,Nsn])
    params['sigma_hSNf']=abs(random.normal(.5,.05,[12]))
    params['P_hh']=[
	random.normal(log10(3),.1), # 1 - t1
	random.normal(log10(80),.1), # 1 - t2
	random.normal(2,.1), # 1 - tp
	...
	random.normal(-2,.1), # 1 - lbetadC
	random.normal(0,.1), # 1 - Mpr
	random.normal(0,.1), # Yb
	random.normal(0,.1), # V
	random.normal(2,.1) # pt0
	]
    params['sigma_hh']=abs(random.normal(.5,.05,[14]))
    params['P_hPf']=random.normal(0,.1,[12,Nfilt,12])
    params['sigma_hPf']=abs(random.normal(0.1,.01,[12]))
    params['P_ht0']=random.normal(0,.1,[12,Nfilt])
    params['sigma_ht0']=abs(random.normal(0.1,.01))
    params['P_hff']=random.normal(0,.1,[12,Nfilt,Nfilt])
    params['sigma_hff']=abs(random.normal(0.1,.01,[12]))
    return params


def pystan_SPII_tm_mod6_init(Nsn,Nfilt=5,fixhyper=0):
    """
    Generate reasonable initial parameters for the SNIIP modified hierarchical light curve model
    that also has explosion date pooling
    ...
    * fixhyper: Unused
    """
    params={}
    params['t_hP']=array([1,10,100,10])
    params['sig_t_hP']=random.normal(1,.01,[4])
    params['t_hF']=random.normal(10,.01,[4,Nfilt])
    params['sig_t_hF']=random.normal(1,.01,[4,Nfilt])
    params['t_hSN']=random.normal(1,.01,[4,Nsn])
    params['sig_t_hSN']=random.normal(1,.01,[4])
    params['t_hFF']=random.normal(1,.01,[4,Nfilt*Nfilt])
    params['sig_t_hFF']=random.normal(1,.01,[4])
    params['t_hPF']=random.normal(1,.01,[4,Nfilt*4])
    params['sig_t_hPF']=random.normal(1,.01,[4])
    params['t1']=random.normal(1,.01,[Nfilt,Nsn])
    params['t2']=random.normal(10,.1,[Nfilt,Nsn])
    params['tp']=random.normal(100,.1,[Nfilt,Nsn])
    params['td']=random.normal(10,.1,[Nfilt,Nsn])
    
    params['r_h']=random.normal(1e-2,1e-4)
    params['sig_r_h']=random.normal(1,.01)
    params['r_hP']=random.normal(1e-2,1e-4,[5])
    params['sig_r_hP']=random.normal(1,.01,[5])
    params['r_hF']=random.normal(1e-2,1e-4,[5,Nfilt])
    params['sig_r_hF']=random.normal(1,.01,[5,Nfilt])
    params['r_hSN']=random.normal(1,.01,[5,Nsn])
    params['sig_r_hSN']=random.normal(1,.01,[5])
    params['r_hFF']=random.normal(1,.01,[5,Nfilt*Nfilt])
    params['sig_r_hFF']=random.normal(1,.01,[5])
    params['r_hPF']=random.normal(1,.01,[5,Nfilt*5])
    params['sig_r_hPF']=random.normal(1,.01,[5])
    params['alpha']=random.normal(1e-2,1e-4,[Nfilt,Nsn])
    params['beta1']=random.normal(1e-2,1e-4,[Nfilt,Nsn])
    params['beta2']=random.normal(1e-2,1e-4,[Nfilt,Nsn])
    params['betadN']=random.normal(1e-2,1e-4,[Nfilt,Nsn])
    params['betadC']=random.normal(1e-2,1e-4,[Nfilt,Nsn])
    
    params['M_h']=random.normal(1,.05)
    params['sig_M_h']=random.normal(.05,.005)
    params['M_hF']=random.normal(1,.05,[Nfilt])
    params['sig_M_hF']=random.normal(.05,.005,[Nfilt])
    params['M_hSN']=random.normal(1,.01,[Nsn])
    params['sig_M_hSN']=random.normal(1,.01)
    params['M_hFF']=random.normal(1,.01,[Nfilt*Nfilt])
    params['sig_M_hFF']=random.normal(1,.01)
    params['Mpr']=random.normal(1,.05,[Nfilt,Nsn])
    
    params['Y_h']=random.normal(0,1)
    params['sig_Y_h']=random.normal(.3,.03)
    params['Ybr']=random.normal(0,1,[Nfilt,Nsn])
    
    params['t0s_h']=random.normal(-2,1e-2)
    params['sig_t0s_h']=random.normal(1e-2,1e-3)
    params['t0l_h']=random.normal(-50,10)
    params['sig_t0l_h']=random.normal(10,1)
    params['pt0']=random.normal(-10,1,[Nsn])
        
    params['V_h']=random.normal(2,.1)
    params['V_hF']=random.normal(2,.1,[Nfilt])
    params['V_hSN']=random.normal(1,.01,[Nsn])
    params['Vr']=random.normal(2,.1,[Nfilt,Nsn])
...
def mat_SPII(t,fit,MJD=0):
    ...
    """
    if MJD: p_t=(tile(t,(len(fit['alpha']),1)).T - tile(fit['t0'],(len(t),1)))
    else:  p_t=(tile(t,(len(fit['alpha']),1)).T)
    ...
    m_rise=(p_Mf/(p_tp)**p_alpha * (p_t)**p_alpha) * ((p_t<p_tp) & (p_t>0))
    m_rise[isnan(m_rise)]=0
    m_plateau=(p_Mf * exp(-p_beta*(p_t-p_tp))) * ((p_t>p_tp) & (p_t<p_tp+p_td))
    m_late=(p_Mf * exp(-p_beta*(p_td)) * exp(-p_betad*(p_t-p_tp-p_td))) * (p_t>(p_tp+p_td))
...
def mat_SPII_m(t,fit,b,MJD=0,z=None,fluxscale=1e7):
    ...
    j='grizy'.find(b)
    t=array(t)
    if MJD: p_t=(tile(t.astype('float'),(len(fit['t1'][:,j]),1)).T - tile(fit['t0'],(len(t),1)))
    else:  p_t=(tile(t.astype('float'),(len(fit['t1'][:,j]),1)).T)
    ...
    m_rise[isnan(m_rise)]=0
    m_slowrise = (p_M1 * exp(p_beta1*(p_t-p_t1))) * ((p_t>p_t1) & (p_t<p_tp+p_t1))
    m_plateau =  (p_Mp * exp(-p_beta2*(p_t-p_t1-p_tp))) * ((p_t>p_tp+p_t1) & (p_t<p_t2+p_tp+p_t1))
    m_lateNi = (p_M2 * exp(-p_betadN*(p_t-p_t1-p_tp-p_t2))) * ((p_t>p_t2+p_tp+p_t1) & (p_t<p_td+p_t2+p_tp+p_t1))
    m_lateCo = (p_Md * exp(-p_betadC*(p_t-p_t1-p_tp-p_t2-p_td))) * (p_t>p_t2+p_tp+p_t1+p_td)
    
    return fluxscale * (m_rise+m_slowrise+m_plateau+m_lateNi+m_lateCo).T
...
def mat_SPII_q(t,f,df,fit):
    ...
    pq_fg = scipy.stats.norm.pdf(f,d_f,sqrt(df**2 + matrix(fit['V']).T)) # probability under data model
    pq_bg = scipy.stats.norm.pdf(f,matrix(fit['Yb']).T,sqrt((df**2 + matrix(fit['Vb']).T))) # probability under outlier model
...
def SPII_dpeak_brightest(name):
    ...
    flens={}
    for b in 'grizy':
	sel=where((aphot['limit']==0) & (aphot['event']==name) & (aphot['band']==b))
	flens[b]=len(sel[0])
    fmost=flens.keys()[argmax(flens.values())]
    if fmost>2:
	sel=where((aphot['limit']==0) & (aphot['event']==name) & (aphot['band']==fmost))
...
def SPII_dpeak_first(name):
	...
	sel=where((aphot['event']==name) & (aphot['limit']==0) & ((aphot['band']=='g') | (aphot['band']=='r') | (aphot['band']=='i') | (aphot['band']=='z')))
	m_max=min(aphot['m'][sel])
	## Find clusters of detections
	sel=where((aphot['event']==name) & (aphot['limit']==0) & ((aphot['band']=='g') | (aphot['band']=='r') | (aphot['band']=='i') | (aphot['band']=='z')) & (aphot['m'] < m_max + 2))
	t=aphot['MJD'][sel]
...
def pystan_SPII_tm_getdseason(events,thresh=20,dp_func=SPII_dpeak_first):
    ...
    * dp_func: Function to use to identify the date of the first detection (SPII_dpeak_first by default)
	...
	if events[i] == '2013-D-500012':
	    ...
	    peds = aphot['MJD'][(aphot['event']==events[i]) & (aphot['MJD'] < ed)]
...
def SPII_plot_flux(name,b,dpeak,fit,MJD=0,plotlines=0,multi=0,ML=0,dt=1,ax=None,plot_median=1,plot_contours=1,mag=0,c_med='k',c_ML='r',usedetlims=1,Kcor=1):
    """
    Optional Parameters:
    ...
    * Kcor: Apply K-corrections to data
    """
    # Load data
    sel=where((aphot['event']==name) & (aphot['band']==b) & (aphot['MJD']>dpeak-50) & (aphot['MJD']<dpeak+300))
    z=aphot[sel]['z'][0]
	...
	if mag==0: ## convert back to flux
	    magmod = distmod(z) + get_r_mzero(name,SPII_dpeak_first(name),band=b)
	    p_m = 10**( (p_m - magmod) / -2.5)
    else:
	if mag:
	    raise Exception("Magnitude plots for single-band light curve fits not implemented")
	else:
    ...
    if MJD:
	p_t += (p_t-median(fit['t0']))*z # note this needs to be z, not 1+z, since it's a correction
    ...
    if ML:
	ax.plot(p_t,p_m[argmax(fit['lp__'])],alpha=0.5,c=c_ML,lw=2,ls='dashed')
	...
	##detections
	ax.errorbar(t[sel_l==0],aphot['m'][sel_d][sel_l==0]+distmod(z)+array(Kc)[sel_l==0],aphot['dm'][sel_d][sel_l==0],fmt='o',mec='none',color=bandc(b,col=1),capsize=0)
	##limits
	if usedetlims: lms=PS1_detlims[b]*ones(sum(sel_l))
	else: lms=aphot['m'][sel_d][sel_l]
	
	ax.plot(t[sel_l],lms+distmod(z),'v',mec='none',color=bandc(b,col=1),ms=2**2)
    else:
	Kc=10**(Kc/-2.5)
	ax.errorbar(t,aphot['raw_flux'][sel_d]*Kc,aphot['raw_flux_err'][sel_d]*Kc,fmt='o',mec='none',color=bandc(b,col=1),capsize=0)
    
    ## Show background fit
    if mag==0:
	pYb=percentile(fit['Yb']*1e7/10**(magmod/-2.5),[16,84])
	ax.fill_between([min(p_t)-100,max(p_t)+100],[pYb[0],pYb[0]],[pYb[1],pYb[1]],color=bandc(b,col=1),alpha=0.2)
    ...
    if MJD: 
	if mag:
	    ax.axis([median(fit['t0'])-30,median(fit['t0'])+300,-14,-20])
	else:
	    ax.axis([median(fit['t0'])-30,median(fit['t0'])+300,-5000,max(p_m_u2)*1.2])
    else:
	if mag:
...
def SPII_plot_mag(name,b,dpeak,fit,plotlines=0,multi=0,ML=0,dt=1):
    ...
    * ML: Plot maximum likelihood model with dashed red line
    """
    # Load data
    sel=where((aphot['event']==name) & (aphot['band']==b) & (aphot['MJD']>dpeak-50) & (aphot['MJD']<dpeak+500))
    z=aphot[sel]['z'][0]
    ...
    ## Plot maximum likelihood model
    if ML:
    	plt.plot(p_t,p_m[argmax(fit['lp__'])],alpha=0.5,c='r',lw=2,ls='dashed')## Plot data
    
    sel_d=where((aphot['band']==b) & (aphot['event']==name) & (aphot['limit']==0))
    t=aphot['MJD'][sel_d]
    plt.errorbar(t,aphot['m'][sel_d]+distmod(z),aphot['dm'][sel_d],fmt='o',mec='none',color=bandc(b,col=1),capsize=0)
    ## Show non-detections
    sel=where((aphot['band']==b) & (aphot['event']==name) & (aphot['limit']==1))
    for i in sel[0]:
...
def get_r_mzero(name,dpeak,band='r'):
    ...
    """
    sel=where((aphot['event']==name) & (aphot['MJD']>dpeak-50) & (aphot['MJD']<dpeak+300) & (aphot['band']==band) & (aphot['limit']==0))
...
def SNquickload(lname,getfit=0):
    ...
    ## Load data
    sel=where((aphot['event']==name) & (aphot['MJD']>dpeak-50) & (aphot['MJD']<dpeak+300) & ((aphot['band']=='g') | (aphot['band']=='r') | (aphot['band']=='i') | (aphot['band']=='z') | (aphot['band']=='y')))
    t=aphot[sel]['MJD']
    m=aphot[sel]['m']
    dm=aphot[sel]['dm']
    ...
    if getfit: 
	global fit
	fit=np.load(rootdir+'mod5_lin_ind/fit4_'+name+'.npy')[1] #.item(0)
...
def get_fit_SN(sfit,snames,name):
    ...
    if len(n)==0: raise Exception("get_fit_SN: Name '"+name+"' not in snames")
    for key in sfit:
	s=shape(sfit[key])
...
def get_indfit_SN(snames,file_name=[rootdir+'mod5_lin_ind/fit_resubmit_','.npy']):
    """
    ...
    * file_name: list of [prefix,suffix] to append to file names for loading
	...
	if i == 0:
		...
		if (len(shape(b)) == 2) or (shape(b)[-1] == shape(sfit[p])[-1]):
...
def getCmdStanSamples(gstring,warmup=0,nodiv=1):
    ...
    * nodiv: If yes, remove divergent samples before output
	...
	## Remove header
	nohead_file = fs.rsplit('/',1)[0] + '/' + 'nohead_' + fs.rsplit('/',1)[1]
	...
	if outarray == None:
		...
		for p in outarray.keys():
		    ...
		    nshape[0] = N
		    outarray[p] = np.concatenate([outarray[p],np.zeros(nshape)],axis=0)
	#### Read in data
	if N>0:
		...
		if len(pdim[p]) == 1:
			...
			outarray[p][crange[0]:crange[1],j] = fdata[p+'.'+str(j+1)][-N:]
		elif len(pdim[p]) == 3:
		    for j in range(pdim[p][1]):
			for k in range(pdim[p][2]):
			    outarray[p][crange[0]:crange[1],j,k] = fdata[p+'.'+str(j+1)+'.'+str(k+1)][-N:]
...
def SNstat_dm15(fit,z,name,band='r',dt=0.1,tmax=150):
	...
	sel=where(pt < tmax - 15*dt) #don't search past the end of the grid
	pmax = argmax(-pm[:,sel[0]],axis=1)
	
	dm15=pm[range(len(pm)),pmax] - pm[range(len(pm)),pmax+int(15/dt)]
...
def SNstat_Lbolo(fit,tt,z,name,bands='griz',AV_cor=1):
    ...
    * AV_cor: If true, apply extinction correction using Kasen's redenning formula
	...
	## Call model
	allLC[bands[j]]=mat_SPII_m_mag(name,tt,fit,bands[j],MJD=0) - distmod(z)
    ...
    ##Step through bands
    for j in range(len(bands)):
	flc+=10**((allLC[bands[j]] + np.meshgrid(offset[j],tt)[0].T)/-2.5) * allbands[bands[j]][1]
...
thed=[('Mi','float'),('Z','float'),('Mf','float'),('R0','float'),('MFe','float'),('XHe','float'),('~Mej','float')]
Kasen_T1_data=[\
#Mi	Z	Mf	R0	MFe	XHe
...
Pastorello_F13=np.genfromtxt(rootdir+'literature/Pastorello04/Figure13.txt')
...
def Pastorello_calc_MNi(fit,z,name,t_min=125,t_max=None):
    ...
    * t_min,t_max: Minimum and maximum time for grid search(days since explosion)
    ...
    ## Get t_max
    if t_max==None:
	t_all=aphot['MJD'][where((aphot['limit']==0) & (aphot['event']==name))]
	t_max=max(t_all)
...
def Kasen_Eq11(L50,t_p,M_Ni,verbose=0,expand_extrap=0):
    ...
    * tp0: True plateau duration (in rest frame days corrected for M_Ni contamination)
	...
	Eq3=t_p - tp0*(1 + 22. * M_Ni * E51**-0.5 * (R0/500.)**-1 * Mej**0.5)**(1/6.) # note: .35 -> 22 to fix Kasen's Eq. 13, see notes from 1/11/2014
	return Eq3**2
    ## Define constraints
    C1=lambda p: p[0] - (min(Kasen_T1['Mi'])/10.-expand_extrap)  # Min > 12
    C2=lambda p: (max(Kasen_T1['Mi'])/10.+expand_extrap) - p[0]  # Min <25
...
def Kasen_calc(name,fit,N=100,allback=1,optreturns=1,bolo_correct=1.,AV_cor=True,expand_extrap=0):
    """
    ...
    * M_Ni
    ...
    Min=np.zeros(N);R0=np.zeros(N);Mej=np.zeros(N);E51=np.zeros(N);tp0=np.zeros(N)
    js=random.randint(0,len(L50),N)
    for i in range(N):
	j=js[i]
	Min[i],R0[i],Mej[i],E51[i],tp0[i]=Kasen_Eq11(log10(L50[j]),t_p[j],M_Ni[j],expand_extrap=expand_extrap)
...
def Kasen_EBV(name,fit,z):
    ...
    U,B,V,R,I=toUBVRI_99em(nan,mg,mr,mi,mz,AB=0) # Assuming Kasen used Vega mags
    Vs=V[:,0]
    Is=I[:,0]
...
Kdic={}
for i in range(1,1+len(allevents)): Kdic[allevents[i-1]]=i
...
for k in range(len(allevents)):
    ...
    for j in range(5):
	try: old_mzs[k,j]=get_r_mzero(allevents[k],dp,band='grizy'[j])
	except: old_mzs[k,j]=0
## Apply correction
fcorrect=10**((old_mzs[K-1,J-1]-30)/-2.5)
fcorrect[abs(fcorrect)>10]=0 # some objects are messed up - they have all non-detections and therefore no real zero point definition, so don't touch them

#fcorrect=10**(((aphot['m'] - -2.5*log10(aphot['raw_flux']))-30)/-2.5)
#fcorrect[abs(fcorrect)>1e4]=0
...
PS1_detlims={}
for b in PS1_bands:
    m=percentile(aphot['m'][where((aphot['band']==b) & (aphot['limit']==0))],95)
    PS1_detlims[b]=m
...
################################
def runstan(model,nobjs='all',samples=20e3,chains=5,parallel=5,delta=.25,thin=10,sinit='myprior',fixstep=0,get_neff=1,max_tree=15,startstep=None,indmodel=0,tdict=ttype,**kwargs):
	...
	if startstep==None:
	    ...
	    mcontrol = {'adapt_delta':delta,'max_treedepth':max_tree,'stepsize':startstep}
	mwarmup = samples / 2
    else:
	mcontrol = {'adapt_engaged':False,'stepsize':fixstep,'max_treedepth':max_tree}
	mwarmup = 0
    
    a=pystan_SPII_tm(subdata,stan_init=sinit,stanmodel=(rootdir+'stan/'+model),chains=chains,iterations=samples,Kcor=Kcors,get_neff=get_neff,control=mcontrol,thin=thin,parallel=parallel,fixhyper=0,warmup=mwarmup,**kwargs)
...
def runstan_ind(model,name,samples=20e3,chains=5,parallel=5,delta=.25,thin=10,sinit='myprior',fixstep=0,get_neff=1,max_tree=15,startstep=None,indmodel=0,saveit=0,**kwargs):
    cond=[]
    ts=arange(0,200)
    ...
    a = pystan_SPII_tm_ind(subdata,stan_init=sinit,stanmodel=(rootdir+'stan/'+model),chains=chains,iterations=samples,Kcor=Kcors,get_neff=get_neff,control=mcontrol,thin=thin,parallel=parallel,warmup=mwarmup,**kwargs)
    if saveit != 0:
    ...
    for j in range(5):
	try:
	    SPII_plot_flux(name,'grizy'[j],dpeak,sfit,plotlines=25,plot_median=0,Kcor=1,MJD=1,multi=1)
	    if saveit !=0: plt.savefig(rootdir+saveit+name+'_'+'grizy'[j]+'.pdf')
	except:
...
def plotrandomLCs(N=10,a=None,**kwargs):
	...
	try:
	    SPII_plot_tm(snames[spick],bpick,[snames,sfit],plotlines=25,plot_median=0,multi=1,Kcor=1,MJD=1,**kwargs)
	except:
...
def testdiv(data):
	...
	bins=np.linspace(min(samples)-.1*abs(min(samples)),max(samples)+.1*(max(samples)),25)
...
def getLCp(key,j,b,oktype='',bands='grizy',eventsel=allevents,typedic=ttype_u): 
    """
    ...
    * j=2: +1 sigma
    """
    if b==None:
	return array([LCpars[name][key][j] for name in eventsel if ((typedic[name]=='SN'+oktype) or (oktype==''))])
    else:
	return array([LCpars[name][key][b][j] for name in eventsel if ((typedic[name]=='SN'+oktype) or (oktype==''))])

def plotphase(key1,key2,b1=None,b2=None,cut=None,fig=None,ax=None,oktypes=['IIp','IIn','IIb','IIl'],okcolors=['r','purple','b','orange','k'],alpha=1.,returndata=0,eventsel=allevents):
    """
    ...
    * eventsel: Event selection array to pass to getLCp
	...
	ax.errorbar(x[sel],y[sel],xerr=[dx1[sel],dx2[sel]],yerr=[dy1[sel],dy2[sel]],fmt='o',label=l,capsize=0,mew=0,color=okcolors[i],alpha=alpha)
...
def fmtsex(text,dec=0):
    ...
    * dec: Set this to append a sign to the DEC
    """
    ##catch h m s format
    if 'h' in text or 'd' in text: text=text.replace('h',':').replace('m',':').replace('s','').replace('d',':')
    ##already has colons?
    if ':' in text: outtext=':'.join([obj.zfill(2) for obj in text.split(':')])
    ##space seperated?
    else: outtext=':'.join([obj.zfill(2) for obj in text.split()])
...
def sextodegl(RAlist,DEClist):
    ...
    for i in range(len(RAlist)):
        try:
            RA=RAlist[i];DEC=DEClist[i]
            [newRA,newDEC]=sextodeg(RA,DEC)
            RAd.append(newRA);DECd.append(newDEC)
        except:
            print 'Failed on #'+str(i)
            RAd.append(nan);DECd.append(nan)
    return [RAd,DECd]
...
def eqDist(RA,DEC,RA0,DEC0,unit='min'):
    ...
    try: radeg=float(RA)        
    except: radeg=sum(array(RA.split(':'),'f')*[1,1/60.,1/(60*60.)])*15
    try: decdeg=float(DEC)
    except: decdeg=sum(array(DEC.split(':'),'f')*[1,1/60.,1/(60*60.)])
    try: ra0deg=float(RA0)
    except: ra0deg=sum(array(RA0.split(':'),'f')*[1,1/60.,1/(60*60.)])*15
    try: dec0deg=float(DEC0)
    except: dec0deg=sum(array(DEC0.split(':'),'f')*[1,1/60.,1/(60*60.)])
    ##radians
    rarad=radeg*pi/180;decrad=decdeg*pi/180
    ra0rad=ra0deg*pi/180;dec0rad=dec0deg*pi/180
    ##calculate distance
    delR=rarad-ra0rad
    delD=decrad-dec0rad
    top=cos(dec0rad)**2*sin(delR)**2+(cos(decrad)*sin(dec0rad)-sin(decrad)*cos(dec0rad)*cos(delR))**2
    bottom=sin(decrad)*sin(dec0rad)+cos(decrad)*cos(dec0rad)*cos(delR)
...
def PS1namer(RA,DEC):
    ...
    return 'PSO J' + (sep_ra[0]).zfill(3) + '.' + sep_ra[1][0:4] + d_sign + (sep_dec[0]).zfill(2) + '.' + sep_dec[1][0:4]
