...
ml_sfit = getCmdStanSamples(rootdir + 'stan/mod7_4/samples_mod7_?_?.csv',
                            warmup=0,
                            nodiv=0)
...
writetex(ml_valfile, 'Nsampcoldrate',
         '%0.2f' % (len(ml_sfit['lp__']) / (Nchains * runtime)))
writetex(ml_valfile, 'Nsampcoldperchain',
         '%0.0f' % (len(ml_sfit['lp__']) / (Nchains)))
...
transformed_parameters = [
    'dm', 'mm', 'pt0', 't1', 't2', 'td', 'tp', 'lalpha', 'lbeta1', 'lbeta2',
    'lbetadN', 'lbetadC', 'Mp', 'Yb', 'V', 'M1', 'M2', 'Md'
]
...
## grab object names
for name in ml_snames:
    textransfer(valfile, ml_valfile, 'PSO' + numsafe(name.split('-')[-1]))
...
s = ax.scatter(ml_sfit['stepsize__'],
               ml_sfit['n_leapfrog__'],
               marker='.',
               s=12,
               c=((ml_sfit['treedepth__'] - 1) / max_treedepth),
               vmin=-0.001,
               vmax=1.001,
               cmap=CB2cm['OrSB'],
               edgecolors='none')
...
writetex(ml_valfile, 'betatwotracepick',
         pso_shortname_dict[ml_snames[pickgood]])
...
ax.yaxis.set_minor_locator(
    matplotlib.ticker.FixedLocator(arange(-5e4, 3e4, 2e3)))
plt.axvline(Nwarmup, c='r')
...
ml_sfit_warm = getCmdStanSamples(rootdir + 'stan/mod7_4/samples_mod7_?_?.csv',
                                 warmup=50,
                                 nodiv=0)

writetex(ml_valfile, 'Nsampwarm', len(ml_sfit_warm['lp__']))

writetex(ml_valfile, 'rhPtracerhat',
         '%0.2f' % calc_rhat(ml_sfit_warm, 'r_hP', takelast=None, sdim1=2))
writetex(
    ml_valfile, 'betatwotracepickrhat', '%0.2f' %
    calc_rhat(ml_sfit_warm, 'lbeta2', takelast=None, sdim1=pickgood, sdim2=1))
...
ppc_picks = [('2011-K-330027', 'g'), ('2012-H-420393', 'r'),
             ('2011-A-120215', 'z'), ('2012-C-370519', 'y')]
for i in range(len(ppc_picks)):
    ...
    SPII_plot_flux(name,
                   ...
                   Kcor=1,
                   mag=1)
    axs[0, i].set_title('abcdef'[i] + '. ' + pso_name_dict[name] + ' $' + b +
                        '$-band',
                        size=10)
    axs[0, i].set_xlabel('')
    axs[0, i].set_ylabel('')
...
ax.hist2d(log(ml_sfit_warm['sig_r_hSNF'][:, 2, 2]),
          ml_sfit_warm['r_hSNF'][:, 2, 2],
          bins=40,
          cmap=cm.gist_heat_r)
plt.xlabel('$\log~\sigma r_{hSNF,\\beta_2}$')
plt.ylabel('$r_{hSNF,\\beta_2}$')
ax.xaxis.set_minor_locator(matplotlib.ticker.FixedLocator(arange(-10, 10,
                                                                 0.1)))
ax.yaxis.set_minor_locator(matplotlib.ticker.FixedLocator(arange(-10, 10,
                                                                 0.1)))
...
fig, ax = plt.subplots(1, figsize=(4, 4))
plt.subplots_adjust(bottom=0.15, left=0.2)
ax.hist2d(ml_sfit_warm['t_hP'][:, 2],
          ml_sfit_warm['t_hF'][:, 2, 2],
          bins=20,
          cmap=cm.gist_heat_r)
plt.xlabel('$t_{hP,t_2}$')
plt.ylabel('$t_{hF,t_2,i}$')
ax.xaxis.set_minor_locator(
    matplotlib.ticker.FixedLocator(arange(-10, 10, 0.01)))
ax.yaxis.set_minor_locator(
    matplotlib.ticker.FixedLocator(arange(-10, 10, 0.02)))
...
sf_htp = (log(10) + ml_sfit_warm['t_hP'][:, 1] +
          ml_sfit_warm['sig_t_hP'][:, 1] *
          (ml_sfit_warm['t_hF'][:, 1, 1] * ml_sfit_warm['sig_t_hF'][:, 1, 1]))
sf_ht2 = (log(100) + ml_sfit_warm['t_hP'][:, 2] +
          ml_sfit_warm['sig_t_hP'][:, 2] *
          (ml_sfit_warm['t_hF'][:, 2, 1] * ml_sfit_warm['sig_t_hF'][:, 2, 1]))

sf_s_htp = (ml_sfit_warm['sig_t_hP'][:, 1] * ml_sfit_warm['sig_t_hF'][:, 1, 1])
sf_s_ht2 = (ml_sfit_warm['sig_t_hP'][:, 2] * ml_sfit_warm['sig_t_hF'][:, 2, 1])

ind_tpf = getLCp('tplateau', 1, 1, eventsel=ml_snames)[where(
    getLCp('tplateau', 2, 1, eventsel=ml_snames) < 15)]
...
for i in range(Nplot):
    ...
    cond1 = (abs(sf_htp - sf_htp[pick[i]]) < std(sf_htp) / 10.) & (
        abs(sf_ht2 - sf_ht2[pick[i]]) < std(sf_ht2) / 10.)
    cond2 = (abs(sf_ht2 - sf_ht2[pick[i]]) < std(sf_ht2) / 10.) & (
        abs(sf_ht2 - sf_ht2[pick[i]]) < std(sf_ht2) / 10.)
...
dist_std = np.zeros(Nmc)
dist_mean = np.zeros(Nmc)
for i in range(Nmc):
    thispick = np.random.randint(0, len(sf_htp), 1)
    slm = sum_of_lognorm([sf_htp[thispick], sf_ht2[thispick]],
                         [sf_s_htp[thispick], sf_s_ht2[thispick]])
...
for i in range(Nplot):
    slm = sum_of_lognorm([sf_htp[pick[i]], sf_ht2[pick[i]]],
                         [sf_s_htp[pick[i]], sf_s_ht2[pick[i]]])
...
axs[0].xaxis.set_minor_locator(
    matplotlib.ticker.FixedLocator(arange(0, 300, 5)))
...
axs_i_0.xaxis.set_minor_locator(
    matplotlib.ticker.FixedLocator(arange(0, 500, 5)))
...
axs_i_1.xaxis.set_minor_locator(
    matplotlib.ticker.FixedLocator(arange(0, 500, 10)))
...
writetex(ml_valfile, 'tplatAboveMaxPten',
         '%0.0f' % (100 * sum(fracabove > 0.1) / float(Nmc)))
writetex(ml_valfile, 'tplatAboveMaxPtwen',
         '%0.0f' % (100 * sum(fracabove > 0.2) / float(Nmc)))
writetex(ml_valfile, 'tplatHIERdiststdmed', '%0.0f' % median(dist_std))
writetex(ml_valfile, 'tplatHIERdiststddown',
         '%0.0f' % percentile(dist_std, 16))
...
writetex(ml_valfile, 'tplatHIERdistmeandown',
         '%0.0f' % percentile(dist_mean, 16))
writetex(ml_valfile, 'tplatHIERdistmeanup',
         '%0.0f' % percentile(dist_mean, 84))
