...
def SNplotMLC(name,
    ...
    * addDM: Add distance modulus (plot abs mag; default = 0)
    ...
    sel = where((aphot['event'] == name) & (aphot['band'] == 'r')
                & (aphot['limit'] == 0))
    dmax = aphot['MJD'][sel][argmin(aphot['m'][sel])]
    vmax = aphot['m'][sel][argmin(aphot['m'][sel])] + dm

    ## Draw labels
    ptitle = pso_shortname_dict[name] + (': ' +
                                         aphot['type'][sel][0]) * addtype
    ...
    for band in bands:
        ...
        ## Detections
        sel = where((aphot['event'] == name) & (aphot['band'] == band)
                    & (aphot['limit'] == 0))
        if len(sel[0]) > 0:
            ax.errorbar(aphot['MJD'][sel],
                        aphot['m'][sel] + dm,
                        aphot['dm'][sel],
                        color=bandc(band[0:3], col=1),
                        mec='none',
                        capsize=0,
                        fmt='.')
        ax.plot(aphot['MJD'][sel],
                aphot['m'][sel] + dm,
                'o',
                label='$' + band.replace('_APER', '') + '$',
                color=bandc(band[0:3], col=1),
                mec='none',
                ms=cs * 5)
        ## Upper limits
        ...
        if usedetlims:
            ax.plot(aphot['MJD'][sel],
                    array([PS1_detlims[band]] * len(sel[0])) + dm,
                    'v',
                    color=bandc(band[0:3], col=1),
                    mec='none',
                    ms=cs * 3)
    ...
    if UV_base:
        for band in ['NUV', 'FUV']:
            sel = where((aphot['event'] == name) & (aphot['band'] == band)
                        & (aphot['limit'] == 0)
                        & (abs(dmax - aphot['MJD']) > 50))
            if len(sel[0]) > 0:
                bm = aphot['m'][sel]
                ax.fill_between(ax.get_xlim(),
                                array([percentile(bm, 16)] * 2) + dm,
                                array([percentile(bm, 84)] * 2) + dm,
                                color='b',
                                alpha=.7)

    ##Legend
    ax.legend(numpoints=1,
              bbox_to_anchor=(.3, 1, .7, .1),
              ncol=5,
              prop={'size': 9})
...
c_Rsun = 6.96e10  # cm
c_pc = 3.086e18  #cm
fplanck = lambda T, l: 2 * c_h * c_c**2 / l**5 / (exp(c_h * c_c /
                                                      (l * c_kB * T)) - 1)
...
def NS_planck(L, T, l):
    ...
    if T < 1e4: Lnu = 0  ##Why do this?
        ...
        Lnu = Lnumax * (nu / numax)**3 * (exp(c_h * numax / (c_kB * T)) -
                                          1) / (exp(c_h * nu / (c_kB * T)) - 1)
    ## Convert to Llambda
    return Lnu / l**2 * 3e18
...
def mSBO_NS(p, t, l, s, z):
    ...
    * Av: V-band extinction (mag)
    ...
    T_1 = 10 * c_KeV * M**-.22 * R**.12 * E**.23 * (
        t * 24.)**-.36  # Kelvin, planar phase
    T_2 = 3 * c_KeV * M**-.13 * R**.38 * E**.11 * t**-.56  # Kelvin, spherical phase
    ## Calculate radius for each phase
    R_1 = R * 500  # see S2.2.3
    R_2 = R * 500 * (t / ts)**((5.31 * n**2 + 9.09 * n + 4.25) /
                               ((1.19 * n + 1) * (5.58 * n + 4.25)))  # Eq. 20
    ...
    fF = lambda l, s, T, R: mean(4 * pi * ((R * c_Rsun)**2 * fplanck(
        T,
        linspace(l - s / 2., l + s / 2., 1000) * 1e-8)))
    vfF = vectorize(fF)
...
def pSPII(p, t, m, dm, lim, z):
    ...
    * dm = distance modulus (mag)
    ...
    sel = where(lim == 0)
    diff = (m - mf)**2 / (dm**2 + V)
    plike_1 = log(1 - Pb) + (-(diff[sel]) / 2. - .5 * log(2 * pi *
                                                          (dm[sel]**2 + V))
                             )  ## good points - detections only
    plike_2 = log(Pb) + (-(m - Yb)**2 / (2 * (Vb + dm**2)) -
                         .5 * log(2 * pi * (Vb + dm**2)))  ## bad points - all
    ...
    if t_0 >= max(t[where(lim == 0)]) or min(
            abs(t_0 - t[where(lim == 0)])) > 100:
        priors.append(-inf)
    # Force model to lie below "good" upper limits
    sel = where((lim == 1) & (q == 1))
    if sum(mf[sel] < m[sel]) > 0:
        priors.append(-inf)  # careful with the direction of that inequality!
